...
from ytopt.benchmark.syr2k_exp.plopper.newPlopper import Syr2k_Plopper as Plopper

# Used to locate the kernel for ploppers
import os, sys
...
class Syr2k_Problem(BaseProblem):
    def __init__(self, class_size, **kwargs):
        ...
        self.problem_params = dict((f"p{i}", "categorical") for i in range(6))
        ...
        self.categorical_cast = dict((f"p{i}", "str") for i in range(6))
        kwargs['categorical_cast'] = self.categorical_cast
        # Will need to swap to capitalized parameters
...
__getattr__ = import_method_builder(Syr2k_Problem, inv_lookup, 60)
