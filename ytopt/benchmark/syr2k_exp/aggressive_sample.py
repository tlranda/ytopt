...
def build():
    ...
    parser.add_argument('--n-sample', type=int, default=30, help="Number of samples per batch")
    parser.add_argument('--max-evals', type=int, default=100000, help="Number of samples to collect")
    ...
    parser.add_argument('--output-prefix', type=str, default='distribution', help="Image filename")
    parser.add_argument('--exhaustive', type=str, required=True, help="Detailed file to rank all configurations")
...
def online(targets, data, inputs, args, fname, speed = None, exhaust = None):
    ...
    if args.model != 'random':
        ...
        model = None
    csv_fields = param_names+['objective','predicted','elapsed_sec','exhaust_idx']
    for i in range(len(targets)-1):
        csv_fields.extend([f'objective_{i}',f'predicted_{i}',f'elapsed_sec_{i}',f'exhaust_idx_{i}'])
    ...
    if exhaust is not None:
        ...
        if 'count' not in exhaust.columns:
            exhaust.insert(0,'count',value=0)
    ...
    with open(supplementary_fname, 'w') as suppfile:
        ...
        with open(fname, 'w') as csvfile:
            ...
            if evals_infer is not None:
                updates_counts = 'exhaust_idx' in evals_infer.columns
                for ss in evals_infer.iterrows():
                    csvwriter.writerow(ss[1].values)
                    if updates_counts:
                        exhaust.loc[ss[1]['exhaust_idx'],'count'] += 1
                csvfile.flush()
            ...
            while eval_master < args.max_evals:
                    ...
                    for row in new_sdv.iterrows():
                        ...
                        for target_problem in targets:
                            ...
                            if not args.skip_evals:
                                print(f"Eval {eval_master+1}/{args.max_evals} ", end='\r')
                            ...
                            if exhaust is None:
                                ...
                                exhaust.loc[full_match_idx,'count'] += 1
                ...
                if sum(EFFICIENCY['rejections'].values()) != 0:
                    print(f"RATIO: {EFFICIENCY['generated']/sum(EFFICIENCY['rejections'].values())}")
                else:
                    print(f"RATIO: NO REJECTIONS")
                print(EFFICIENCY['rejections'])
                print(EFFICIENCY['durations'])
        csvfile.close()
    return data.iloc[ORIGINAL_DATA_LENGTH:], exhaust, param_names

def make_plot(args, sampled, exhaustive, param_names):
    x = exhaustive.index
    y = exhaustive['count']
    nonzero = [(idx,val) for (idx,val) in enumerate(y) if val != 0]
    x = [_[0] for _ in nonzero]
    y = [_[1] for _ in nonzero]
    fig, ax = plt.subplots()
    ax.scatter(x,y,label='Sample Frequency')
    ax.set_xlabel("Configuration in Objective-Rank order (lower is better)")
    ax.set_ylabel(f"Sample Frequency out of {args.max_evals} Samples")
    ax.set_title(f"{args.model} Sampling From {', '.join(args.inputs)} @{args.top}")
    ax.set_xlim((0,len(exhaustive)))
    ax.set_ylim((0, max(y)+1))
    fig.savefig(f"{args.output_prefix}_image.png")
